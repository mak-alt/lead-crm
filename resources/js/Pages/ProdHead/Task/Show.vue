                                                ...
                                                <div class="flex-shrink-0">
                                                   <div class="dropdown">
                                                      ...
                                                      <ul class="dropdown-menu" style="">
                                                        <li>
                                                            <Link :href="route('prodhead.tasks.remove.member',{ slug: currentWebsite.slug, task_id: task.id, id: member.id })" class="dropdown-item"><i class="ri-delete-bin-5-fill text-muted me-2 align-bottom"></i>Remove</Link>
                                                        </li>
                                                      </ul>
                                                   </div>
                                                </div>
                ...
                <template #content>
                    <form @submit.prevent="updateDates">
                        <div class="row">
                            ...
                            <div class="col-xl-12 col-md-12">
                                <div class="d-grid">
                                    <button type="submit" class="btn btn-primary btn-block" :disabled="update_dates.processing">Submit</button>
                                </div>
                            </div>
                        </div>
                    </form>
...
<script>
    ...
    import AddMemberModal from '@/Pages/ProdHead/Task/Partials/AddMemberModal'
    import CreateResource from '@/Pages/ProdHead/Task/Partials/CreateResource'
    import Discussion from '@/Pages/ProdHead/Task/Partials/Discussion'
	import DiscussionForm from '@/Pages/ProdHead/Task/Partials/DiscussionForm'
    import UpdateSummaryModal from '@/Pages/ProdHead/Task/Partials/UpdateSummaryModal'
    import UpdateDatesModal from '@/Pages/ProdHead/Task/Partials/UpdateDatesModal'
	...
	export default {
		...
		data() {
			return {
                ...
                remove_member: this.$inertia.form({
                    id: ''
                }),
		...
		methods: {
			...
			updateStatus(e) {
				...
				axios.post(route('prodhead.tasks.update.status', { slug: this.currentWebsite.slug, id: this.task.id }), { stage_id: this.stage_id }).then((response) => {
                    console.log(response.data);
                });
			},
			addMember() {
        		this.add_member.post(route('prodhead.tasks.add.members', { slug: this.currentWebsite.slug, id: this.task.id }),{
        			preserveScroll: true,
        			preserveState: true,
        			onSuccess: () => {
            ...
            removeMember() {
                this.remove_member.post(route('prodhead.tasks.remove.member',{ slug: this.currentWebsite.slug, id: this.task.id }),{
                    preserveScroll: true,
                    preserveState: true,
                    onSuccess: () => {
                        console.log(true);
        	...
        	uploadResource() {
                ...
                this.resource.post(route('prodhead.tasks.upload.media', { slug: this.currentWebsite.slug, id: this.task.id }),{
                    preserveScroll: true,
                    onSuccess: () => {
                        this.resource.myFiles = [];
                        this.resource.reset();
            ...
            updateSummary() {
                this.update_summary.post(route('prodhead.tasks.update.summary', { slug: this.currentWebsite.slug, id: this.task.id }),{
                    preserveScroll: true,
                    onSuccess: () => {
            ...
            updateDates() {
                this.update_dates.post(route('prodhead.tasks.update.dates', { slug: this.currentWebsite.slug, id: this.task.id }),{
                    preserveScroll: true,
                    onSuccess: () => {
            ...
            getDiscussions() {
				axios.get(route('prodhead.tasks.discussions', { slug: this.currentWebsite.slug }), { params: { task_id: this.task.id } }).then((response) => {
			...
			addDiscussion(comment) {
				axios.post(route('prodhead.tasks.discussions.store', { slug: this.currentWebsite.slug }), { task_id: this.task.id, comment: comment.comment }).then((response) => {
                    this.discussions.push(response.data);
                });
                setTimeout(() => this.scrollToBottom(), 1000);
			},
            createSubTask() {
                this.create_subtask.post(route('prodhead.subtasks.store', { slug: this.currentWebsite.slug }),{
                    preserveState: true,
                    preserveScroll: true,
                    onSuccess: () => {
            ...
            updateSubTask() {
                this.update_subtask.post(route('prodhead.subtasks.update', { slug: this.currentWebsite.slug, id: this.update_subtask.id }),{
                    preserveState: true,
                    preserveScroll: true,
                    onSuccess: () => {
            ...
            updateSubtaskStatus(event) {
                ...
                axios.post(route('prodhead.subtasks.status.update', { slug: this.currentWebsite.slug, id: subtask_id }), {is_completed: is_completed }).then((response) => {
