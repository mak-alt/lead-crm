                        ...
                        <div class="card" v-if="user.websites.length > 0">
						   <div class="card-body">
						      ...
						      <div class="d-flex flex-wrap gap-2 fs-15">
						         <Link :href="route('admin.websites.edit',{ id: website.id })" class="badge badge-soft-primary" v-for="website in user.websites">{{ website.name }}</Link>
						      </div>
						   </div>
						   <!-- end card body -->
						</div>
...
<script>
	...
	export default {
		...
		methods: {
			updatePassword() {
                this.password.post(route('admin.profile.update.password', { id: this.user.id }),{
                    preserveScroll: true,
			...
			updateProfileImage(event){
				...
				axios.post(route('admin.profile.update.image', { id: this.user.id }), formData, {
				    headers: {
