...
<script>
    ...
    import AddMemberModal from '@/Pages/Partner/Task/Partials/AddMemberModal'
    import CreateResource from '@/Pages/Partner/Task/Partials/CreateResource'
    import Discussion from '@/Pages/Partner/Task/Partials/Discussion'
	import DiscussionForm from '@/Pages/Partner/Task/Partials/DiscussionForm'
    import UpdateSummaryModal from '@/Pages/Partner/Task/Partials/UpdateSummaryModal'
    import UpdateDatesModal from '@/Pages/Partner/Task/Partials/UpdateDatesModal'
	...
	export default {
		...
		methods: {
			...
			updateStatus(e) {
				...
				axios.post(route('partner.tasks.update.status', { slug: this.currentWebsite.slug, id: this.task.id }), { stage_id: this.stage_id }).then((response) => {
                    console.log(response.data);
                });
			},
			addMember() {
        		this.add_member.post(route('partner.tasks.add.members', { slug: this.currentWebsite.slug, id: this.task.id }),{
        			preserveScroll: true,
        			preserveState: true,
        			onSuccess: () => {
        	...
        	uploadResource() {
                ...
                this.resource.post(route('partner.tasks.upload.media', { slug: this.currentWebsite.slug, id: this.task.id }),{
                    preserveScroll: true,
                    onSuccess: () => {
                        this.resource.myFiles = [];
                        this.resource.reset();
            ...
            updateSummary() {
                this.update_summary.post(route('partner.tasks.update.summary', { slug: this.currentWebsite.slug, id: this.task.id }),{
                    preserveScroll: true,
                    onSuccess: () => {
            ...
            updateDates() {
                this.update_dates.post(route('partner.tasks.update.dates', { slug: this.currentWebsite.slug, id: this.task.id }),{
                    preserveScroll: true,
                    onSuccess: () => {
            ...
            getDiscussions() {
				axios.get(route('partner.tasks.discussions', { slug: this.currentWebsite.slug }), { params: { task_id: this.task.id } }).then((response) => {
			...
			addDiscussion(comment) {
				axios.post(route('partner.tasks.discussions.store', { slug: this.currentWebsite.slug }), { task_id: this.task.id, comment: comment.comment }).then((response) => {
